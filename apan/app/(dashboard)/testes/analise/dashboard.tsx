...
export default function VideoDashboardScreen() {
  ...
  const handleAutoSave = async () => {
    ...
    try {
      ...
      const modalidadeAlvo = modalidades.find(
        (m: any) =>
      ...
      if (!modalidadeAlvo)
        throw new Error("Modalidade 'Salto' não encontrada.");

      // 2. Buscar e Mapear Métricas (SQL)
      const metricasDisponiveis = await DadosAuxiliaresService.getMetricas(
        modalidadeAlvo.id
      );
      ...
      for (const metrica of metricasDisponiveis) {
        ...
        } else if (nome.includes("cadência"))
          valor = Number(aiValues.cadence.toFixed(2));
        else if (nome.includes("distância"))
          valor = Number(aiValues.distance.toFixed(2));
      ...
      if (dataLite.series) {
        ...
        const keysToRemove = Object.keys(dataLite.series).filter(
          (key) =>
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.subtitle}>
        Processamento via Computer Vision (RunPod)
      </Text>
      ...
      <View style={styles.kpiContainer}>
        ...
        <KPICard
          label="Salto"
          value={
            data.jump?.has_jump && data.jump?.jump_height_m
      ...
      <View style={styles.detailsCard}>
        ...
        <Row
          ...
          value={`${data.speed?.distance_m?.toFixed(2) ?? 0} m`}
          theme={theme}
        />
        <Row
          label="Total de Passos"
          value={data.stride?.stride_count?.toString() ?? "0"}
          theme={theme}
        />
        <Row
          label="Comprimento Médio Passada"
          value={
            data.stride?.stride_length_mean_m
              ? `${data.stride.stride_length_mean_m.toFixed(2)} m`
              : "--"
          }
          theme={theme}
        />
        ...
        <Row
          label="Distância"
          value={`${data.speed?.distance_m?.toFixed(2)} m`}
          theme={theme}
        />
        <Row
          label="Passos"
          value={data.stride?.stride_count?.toString()}
          theme={theme}
        />
        {data.jump?.has_jump && (
          <>
            ...
            />
            <Row
              label="Distância Salto (Horiz.)"
              value={`${data.jump.jump_distance_m.toFixed(2)} m`}
              theme={theme}
            />
          </>
          <>
